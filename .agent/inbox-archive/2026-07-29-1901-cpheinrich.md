...
MO-038 while another session held it ([MO-040](product/roadmap/MO-26-07-29-040-pm-new-re-issues.md)). The specification no
...
([MO-041](product/roadmap/MO-26-07-29-041-pm-claim-only-documented.md)). An inbox cycle needs no roadmap item any more — **this
message is the first one riding its own `inbox-2026-07-29` branch** rather than borrowing an
item's ([MO-042](product/roadmap/MO-26-07-29-042-inbox-cycle-has.md)).
...
[MO-010](product/roadmap/MO-26-07-29-010-simplify-architecture-md-first.md) **went to `shipped` with `prs: [31]`** — a PR that changed
...
~ These PRs are looking good, but I would like to preview all of them before merging. I believe I will need to configure the Vercel credentials for these. pcheinrich and heinrichrbros should be on my personal Vercel account, and lakina, well, I might put Lakina on my own vercel if its possible to have project-scoped permissions so that I could invite my friend Alex to the Lakina project on my own Vercel account, or if I want to have collaborator scoped access do I need to use a Lakina account (this may already exist, not sure)
...
~ I have another claude session in evo that is working on this. So let's come back to it. I was honestly a bit distracted when I was working through the evo brand builder so I may need to go back and fixt things up. 
...
~ This might have been me that modified it mid session. Keep an eye on it for now. 
