...
describe("roadmapIdFromBranch", () => {
  ...
  it("extracts a timestamp id (MO-057)", () => {
    ...
    expect(roadmapIdFromBranch("mo-2026-08-01-15.26.34-blocked-is-an-outcome")).toBe(
      "MO-2026-08-01-15.26.34",
    );
    expect(roadmapIdFromBranch("mo-2026-08-01-15.26.34")).toBe("MO-2026-08-01-15.26.34");
  });

  it("extracts a migrated id", () => {
    expect(roadmapIdFromBranch("mo-2026-07-29-045-simplify-architecture")).toBe(
      "MO-2026-07-29-045",
    );
  });
  ...
  it("never returns a truncated prefix of a dated branch", () => {
    ...
    for (const b of ["mo-2026-08-01-15.26.34-x", "ev-2026-12-31-045-y", "cph-2026-01-02-00.00.01"]) {
      expect(roadmapIdFromBranch(b)).not.toMatch(/^[A-Z]{2,4}-\d{4}$/);
    }
  });
