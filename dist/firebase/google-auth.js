...
async function systemRunner(command, args, cwd, options) {
 ...
 */
export function expectedRedirectUris(_project, domain) {
...
async function gcloudToken(runner, root, allowBrowserLogin) {
    try {
        const { stdout } = await runner("gcloud", ["auth", "print-access-token"], root, { timeoutMs: NETWORK_TIMEOUT_MS });
        if (stdout.trim())
            return stdout.trim();
    }
    catch (error) {
...
async function gcloudEmail(runner, root) {
    const { stdout } = await runner("gcloud", ["config", "get-value", "account"], root, { timeoutMs: NETWORK_TIMEOUT_MS });
...
async function ensureFirebaseLogin(runner, root, allowBrowserLogin) {
    try {
        await runner("firebase", ["projects:list", "--json"], root, { timeoutMs: NETWORK_TIMEOUT_MS });
        return;
    }
    catch {
...
export async function setupGoogleAuth(opts) {
    ...
    try {
        ...
        try {
            ...
            throw new Error(`Firebase Auth deploy failed (${errorMessage(error)}) and firebase.json could not be restored: ${errorMessage(rollbackError)}`);
        }
        return throwSetupFailure(error, opts, runner);
    }
    try {
