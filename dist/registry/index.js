...
export async function readRegistry(path = registryPath()) {
    try {
...
export async function writeRegistry(reg, path = registryPath()) {
...
export function suggestPrefix(name, taken) {
...
export async function addProject(opts, path = registryPath()) {
    const reg = await readRegistry(path);
    const abs = resolve(opts.path);
    const clash = reg.projects.find((p) => p.prefix === opts.prefix && resolve(p.path) !== abs);
    if (clash) {
        throw new RegistryError(`Prefix ${opts.prefix} is already used by "${clash.name}" (${clash.path}).`);
    }
    const entry = RegisteredProject.parse({ ...opts, path: abs });
    const rest = reg.projects.filter((p) => resolve(p.path) !== abs && p.name !== opts.name);
    const next = { version: 1, projects: [...rest, entry].sort((a, b) => a.name.localeCompare(b.name)) };
    await writeRegistry(next, path);
    return next;
}
export async function removeProject(name, path = registryPath()) {
    const reg = await readRegistry(path);
    const next = {
        version: 1,
...
export function takenPrefixes(reg) {
