...
Apply these as a test: if a design or a sentence does not read as ${list(a.feels)}, it is wrong regardless of whether it is otherwise good.
...
function visualSystem(a, name, prefix) {
 ...
 */
function explorePrompt(a, name, prefix) {
...
**Primary audience:** ${a.primaryAudience}${a.secondaryAudience ? `\n\n**Secondary audience:** ${a.secondaryAudience}` : ""}
...
function plan(answers, name, prefix) {
    const planned = [
 ...
 * look plausible.
 */
export async function generateBrand(brandDir, name, prefix, answers, opts = {}) {
    ...
    for (const { path: rel, content, ownership } of plan(answers, name, prefix)) {
        ...
        if (existing === next)
            continue; // already current — not a skip worth reporting
 ...
 */
export async function checkDrift(brandDir, name, prefix, answers) {
    const derived = [];
    const seeded = [];
    const missing = [];
    for (const { path: rel, content, ownership } of plan(answers, name, prefix)) {
