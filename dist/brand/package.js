...
const exists = async () => null;
async function nonEmpty(dir, path, minimum = 80) {
    try {
...
async function moodboardImages(path) {
    let entries;
    try {
...
function filled(group) {
    if (typeof group !== "object" || group === null)
        return false;
    return Object.keys(group).some((key) => !isAnnotation(key));
}
const checkTokens = async (dir) => {
    ...
    const tokens = doc;
    const empty = ["color", "font", "space"].filter((group) => !filled(tokens[group]));
...
const checkMessaging = async (dir) => {
    let doc;
    try {
    ...
    const record = doc;
    const empty = ["what", "mission", "primaryAudience"].filter((key) => typeof record[key] !== "string" || record[key].trim().length < 5);
    return empty.length ? `no usable ${list(empty)} value${empty.length === 1 ? "" : "s"}` : null;
};
const checkMoodboards = async (dir) => {
...
const checkApplication = async (dir) => {
    ...
    if (!imagery)
        return `${IMAGERY_FILE} is not ready`;
...
export function formatStatus(status, name) {
    const marks = { ok: "\x1b[32m✓\x1b[0m", missing: "\x1b[31m✗\x1b[0m", incomplete: "\x1b[33m~\x1b[0m" };
    const lines = [`\n\x1b[1m${name} — brand workflow\x1b[0m`];
    for (const source of ["scaffold", "input", "exploration", "final"]) {
        const entries = status.required.filter((entry) => entry.source === source);
        if (!entries.length)
            continue;
        const heading = source === "scaffold"
            ? "Workflow"
            : source === "input"
                ? "Exploration input"
                : source === "exploration"
    ...
    if (have.length)
        for (const entry of have)
            lines.push(`  \x1b[32m✓\x1b[0m ${entry.path}`);
