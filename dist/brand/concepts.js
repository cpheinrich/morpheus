 ...
 */
export const conceptReviewMeta = (count = 5) => `<meta name="${CONCEPT_REVIEW_META_NAME}" content="concepts=${count}; views=${CONCEPT_REVIEW_VIEWS.join(",")}">`;
export async function checkConceptReview(dir) {
    let html;
    try {
    ...
    const tag = (html.match(/<meta\b[^>]*>/gi) ?? []).find((candidate) => new RegExp(`\\bname=["']${CONCEPT_REVIEW_META_NAME}["']`, "i").test(candidate));
    ...
    if (!count || Number(count[1]) < 5)
        return "must contain at least five comparable concepts";
    ...
    if (missing.length)
        return `missing ${missing.join(", ")} view${missing.length === 1 ? "" : "s"}`;
    const unmarkedViews = CONCEPT_REVIEW_VIEWS.filter((view) => !new RegExp(`\\b${CONCEPT_REVIEW_VIEW_ATTRIBUTE}=["']${view}["']`, "i").test(html));
