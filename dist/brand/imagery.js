...
export async function readImagery(dir) {
    try {
...
export async function checkImagery(dir) {
    let raw;
    try {
    ...
    const hasLivePlacement = parsed.data.assets.some((asset) => asset.placements.some((placement) => /home|marketing|app|product|hero/i.test(placement)));
