...
import { LEGACY_VIBES_FILE, readVibes, renderVibes, VIBES_FILE, } from "./vibes.js";
...
async function migrateLegacyVibes(brandDir, name) {
...
export async function initializeWorkflow(opts) {
    ...
    const result = await writePlan(opts.brandDir, plan(opts.name, opts.prefix, vibes.text || renderVibes(opts.name)), Boolean(opts.refresh));
    if (!migrated)
        return result;
    return {
