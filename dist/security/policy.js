...
export function smallestFixedVersion(vulnerability, installed) {
...
export function findingsFromOsvJson(value) {
    const input = value;
    if (!Array.isArray(input?.results))
        throw new Error("OSV JSON must contain a results array");
    const findings = [];
    ...
    return findings.filter((finding) => !finding.withdrawn).sort((a, b) => Number(b.malicious) - Number(a.malicious) ||
...
function normalizeSourcePath(path) {
    ...
    if (!direct.startsWith("..") && existsSync(direct))
        return direct;
