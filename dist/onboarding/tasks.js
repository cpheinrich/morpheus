 ...
 */
async function hasValidArtifact(root, kind) {
 ...
 */
async function firebaseGoogleAuthReady(root) {
...
export const TASKS = [
    // ---------------------------------------------------------------- repo ---
    {
        ...
        detect: async (root) => {
            const m = await readJson(join(root, "morpheus.json"));
    ...
    },
    {
        ...
        detect: async (root) => (await Promise.all([".agent/decisions.md", ".agent/learned.md", ".agent/worklog", ".agent/inbox-archive"].map((p) => exists(join(root, p))))).every(Boolean),
    },
    {
        id: "registry",
        group: "Repository",
    ...
    },
    {
        ...
        network: true,
        detect: async (root) => {
            const out = await tryRun("gh", ["api", "repos/{owner}/{repo}/branches/main/protection", "--jq", ".required_status_checks.strict"], root);
    ...
    },
    {
        ...
        detect: async (root) => {
            ...
            try {
                ...
                files = (await readdir(dir)).filter((f) => f.endsWith(".md") && !TEAM_RESERVED.has(f.toLowerCase()));
            }
            catch {
                return false;
            }
    ...
    },
    {
        ...
        detect: async (root) => (await exists(join(root, ".env.example"))) || (await exists(join(root, "apps/web/.env.example"))),
    },
    // --------------------------------------------------------- infra: gcp ---
    {
        id: "gcp-project",
...
export const appliesTo = (t, kind) => !t.kinds || t.kinds.includes(kind);
export const tasksFor = (kind) => TASKS.filter((t) => appliesTo(t, kind));
export const groupsFor = (kind) => [
    ...new Set(tasksFor(kind).map((t) => t.group)),
];
/** The project's declared kind, defaulting the way `doctor` does. */
export async function projectKind(root) {
    const m = await readJson(join(root, "morpheus.json"));
    const k = m?.kind;
 ...
 */
export async function projectLabel(root) {
    const m = await readJson(join(root, "morpheus.json"));
