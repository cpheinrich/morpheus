...
import { groupsFor } from "./tasks.js";
/**
...
const ANCHOR = (id) => `<!-- morpheus:task ${id} -->`;
...
function renderTask(s) {
    ...
    if (s.note)
        lines.push(...s.note.split("\n").map((l) => `  ${l}`));
    return lines.join("\n");
}
export function renderOnboarding(name, statuses, kind) {
...
export async function readOnboarding(root) {
    try {
...
export async function writeOnboarding(root, name, statuses, kind) {
...
export function setState(statuses, id, state) {
    const s = statuses.find((x) => x.task.id === id);
    if (!s)
        return { ok: false, reason: `No task "${id}". Run \`morpheus init status\` to list them.` };
    if (s.detected) {
        return {
            ok: false,
