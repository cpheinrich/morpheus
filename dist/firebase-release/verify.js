...
function sourcePath(value) {
    assert(typeof value === "string" && /^[A-Za-z0-9_./-]+$/.test(value)
        && !value.startsWith("/") && value.split("/").every((part) => part && part !== "." && part !== ".."), "Source paths must be repository-relative without traversal");
    return value;
}
...
export function assertSource(sourceSha, mainSha, checkoutSha, ref) {
...
export function indexKey(value, expected = false) {
    ...
    const fields = index.fields.map((value) => {
        ...
        assert(Object.keys(field).every((key) => ["fieldPath", "order", "arrayConfig"].includes(key)), "Unsupported index field option requires a reviewed readiness implementation");
        assert((field.order === undefined || field.order === "ASCENDING" || field.order === "DESCENDING")
            ...
            && (field.order !== undefined) !== (field.arrayConfig !== undefined), "Each index field must have exactly one supported ordering or array mode");
...
export async function verifyRelease(options) {
    ...
    do {
        ...
        for (const value of (page.indexes ?? [])) {
            const index = object(value);
            if (index.state !== "READY")
                continue;
            assert(typeof index.name === "string" && index.name.startsWith(`projects/${target.project}/databases/(default)/collectionGroups/`), "Missing or cross-project index identity");
            // Unrelated indexes with newer unsupported options cannot satisfy a required index.
            try {
                ready.add(indexKey(index));
            }
            catch { /* Required keys remain missing. */ }
        }
        assert(page.nextPageToken === undefined || typeof page.nextPageToken === "string", "Invalid index page token");
        token = (page.nextPageToken ?? "");
    } while (token);
    for (const key of required)
        assert(ready.has(key), `Required index missing or not READY: ${key}; provision separately and retry`);
...
export function deployConfig(target, sourceFiles) {
