 ...
 */
async function blockTarget(productDir, id, needs) {
    ...
    for (const path of candidatePaths) {
        ...
        if (data["id"] !== id || data["status"] !== "blocked")
            continue;
...
export async function block(opts) {
    ...
    if (!needs.trim()) {
        throw new BlockError(`--needs is required, and "blocked on Chris" is not a need. Say what would ` +
    ...
    const date = today();
    const written = [];
    await writeFile(item.path, updateFrontmatter(item.raw, { status: "blocked", needs: needs.trim(), updated: date }), "utf8");
    written.push(item.path);
    const worklogPath = join(root, ".agent/worklog", `${date}-${id.toLowerCase()}-blocked-${slugify(needs)}.md`);
    ...
    await writeFile(inboxPath, appendOpenItem(existing, {
