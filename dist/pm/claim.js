 ...
 */
export const FETCH_PRUNE = ["fetch", "origin", "--prune", "--quiet"];
/** Branch prefix for an item: EV-014 -> ev-014- */
export function branchPrefix(id) {
...
export async function findClaims(id, cwd) {
    const out = await git(["ls-remote", "--heads", "origin", `refs/heads/${branchPrefix(id)}*`], cwd);
    if (!out)
        return [];
    return out
        .split("\n")
 ...
 */
export async function claimedNumbers(idPrefix, cwd) {
    try {
        const out = await git(["ls-remote", "--heads", "origin", `refs/heads/${idPrefix.toLowerCase()}*`], cwd);
 ...
 */
export function parseClaimedNumbers(lsRemote, idPrefix) {
...
export async function listClaims(cwd) {
 ...
 */
export function parseClaimRefs(forEachRefOutput) {
    const claims = [];
...
async function setStatusInProgress(productDir, id) {
    const { items } = await parseArtifact(productDir, "roadmap");
    const item = items.find((i) => i.data.id === id);
    if (!item)
        throw new ClaimError(`No roadmap item ${id} in ${productDir}/roadmap/`);
    ...
    await writeFile(item.path, updateFrontmatter(raw, { status: "in-progress", needs: null, updated: today() }), "utf8");
}
/**
 ...
 */
async function itemPath(productDir, id) {
    const { items } = await parseArtifact(productDir, "roadmap");
    return items.find((i) => i.data.id === id)?.path ?? null;
}
/**
...
export async function claim(productDir, id, cwd) {
    ...
    if (existing.length > 0) {
        ...
        if (item.data.status === "blocked") {
            throw new ClaimError(`${id} is blocked and still holds its branch — the partial work is on it.\n` +
                `Resume it rather than re-claiming:\n\n` +
                `  git checkout ${existing[0]}\n` +
                `  morpheus pm unblock ${id}\n\n` +
                `It needs: ${item.data.needs ?? "(unrecorded)"}`);
        }
        throw new ClaimError(`${id} is already claimed by branch "${existing[0]}". ` +
    ...
    const shipped = (reconciled?.outcomes ?? [])
        ...
        .map((o) => o.id);
    const shippedPaths = (await Promise.all(shipped.map((sid) => itemPath(productDir, sid)))).filter((p) => p !== null);
