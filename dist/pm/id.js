 ...
 */
export const ROADMAP_ID = /^[A-Z]{2,4}-((?:\d{2}|\d{4})-\d{2}-\d{2}-(?:\d{2}\.\d{2}\.\d{2}|\d{3})|\d{3,})$/;
/** Just the two dated shapes — what `pm new` now produces. */
export const DATED_ROADMAP_ID = /^[A-Z]{2,4}-(?:\d{2}|\d{4})-\d{2}-\d{2}-(?:\d{2}\.\d{2}\.\d{2}|\d{3})$/;
/**
...
export function parseRoadmapId(id) {
 ...
 */
export function periodInZone(d = new Date()) {
    const p = zoned(d);
 ...
 */
export function isoDateInZone(d = new Date()) {
    const p = zoned(d);
 ...
 */
export function timestampId(prefix, taken, now) {
...
export function migratedId(prefix, created, oldNumber) {
    // `2026-07-29` -> `26-07-29`
 ...
 */
export function slugForFilename(title, max = SLUG_MAX) {
    const words = title
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, " ")
        .trim()
    ...
    // nothing.
    if (!out.length)
        return kept[0]?.slice(0, max) ?? "";
    while (out.length > 1 && (STOP_WORDS.has(out.at(-1)) || DANGLING.has(out.at(-1))))
        out.pop();
    return out.join("-");
}
/**
...
export function itemFilename(id, title, max = SLUG_MAX, chosen) {
 ...
 */
export function roadmapIdFromBranch(branch) {
    const dated = /^([a-z]{2,4})-((?:\d{2}|\d{4})-\d{2}-\d{2}-(?:\d{2}\.\d{2}\.\d{2}|\d{3}))(?:-|$)/i.exec(branch);
    if (dated)
        return `${dated[1].toUpperCase()}-${dated[2]}`;
