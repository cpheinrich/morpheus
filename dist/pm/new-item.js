 ...
 * the file.
 */
function sequencePrefix(kind, prefix, now) {
 ...
 */
export async function nextId(productDir, kind, prefix, cwd, now = new Date()) {
    ...
    // suppressed.
    if (kind === "roadmap") {
        const taken = items.map((i) => i.data.id);
...
export async function createItem(opts) {
    ...
    try {
        ...
        if (e.code !== "EEXIST")
            throw e;
        throw new Error(`${path} already exists, so ${id} was not written.\n` +
