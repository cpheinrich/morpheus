...
import { hasSecurityMarker, isSecurityDependencyOnly, MORPHEUS_SECURITY_LOGIN, } from "../security/policy.js";
/**
 ...
 */
export function waiverReason(body, key) {
...
export function closesIssue(body, issue) {
    ...
    return new RegExp(String.raw `(?:^|[^A-Za-z0-9])${keyword}\s+#${issue}(?!\d)`, "im").test(visible);
}
...
export function hasSection(body, heading) {
    ...
    for (const line of lines) {
        ...
        if (m) {
            if (inSection)
                return false; // hit the next heading with nothing between
...
export async function checkPr(ctx) {
    ...
    if (source.length > 0 && tests.length === 0 && !testsWaived) {
        findings.push({
            ...
            message: `${source.length} source file(s) changed with no test changes. Add tests, ` +
    ...
    if (reviewReason !== null && isRealReason(reviewReason)) {
        findings.push({
            level: "waived",
            rule: "review-waived",
            message: `legacy agent-review delivery waived — "${reviewReason}". ` +
        ...
        findings.push({
            level: "error",
            rule: "review-waived",
            message: `"review-waived: ${reviewReason}" is not a reason — the delivery check will refuse it. ` +
    ...
    if (id && hasNoSubstantiveChange(changedFiles)) {
        if (!recordsWaived) {
            findings.push({
                ...
                message: `"${branch}" claims ${id}, but this PR changes only records and board files — ` +
    ...
    if (isRecordsOnly(changedFiles)) {
        ...
        findings.push({
            level: "warning",
            rule: "branch-name",
            message: `Branch "${branch}" does not reference a roadmap item (expected mo-014-slug). ` +
        ...
        if (!item) {
            ...
            if (missingClosures.length) {
                findings.push({
                    level: "error",
                    rule: "issue-closure",
                    message: `${id} declares GitHub issue${missingClosures.length === 1 ? "" : "s"} ` +
            ...
            if (!["review", "shipped"].includes(item.data.status)) {
                findings.push({
                    level: "error",
                    rule: "roadmap-status",
                    message: item.data.status === "blocked"
    ...
    const staleContext = (ctx.trunkChanges ?? []).filter((f) => CANONICAL.test(f));
    if (staleContext.length) {
        findings.push({
            level: "warning",
            rule: "context-drift",
            message: `Canonical records changed on the base branch while this one was open: ` +
...
export function formatFindings(findings) {
    ...
    if (rest.length === 0 && waived.length === 0)
        return "✓ PR conventions satisfied.";
