...
async function idtk(path, project, token, body) {
...
export async function lookupUsers(project, token, emails) {
    if (emails.length === 0)
        return new Map();
    const { users = [] } = await idtk("/accounts:lookup", project, token, { email: emails });
...
function currentRole(user) {
    if (!user.customAttributes)
        return undefined;
    try {
        return JSON.parse(user.customAttributes).role;
    }
    catch {
        return undefined;
    }
}
export async function syncAccess(opts) {
    ...
    if (revokeUnlisted) {
        ...
        const all = await idtk("/accounts:query", project, token, {});
