...
export const EXPECTED = {
    company: [
        ...
        "hq/brand",
        "hq/marketing/seo",
        "hq/marketing/instagram",
        "hq/marketing/linkedin",
        "hq/marketing/x",
        "hq/marketing/reddit",
        "hq/finance",
        "hq/ops",
 ...
 */
async function checkRequiredRecords(root, add, covered = []) {
    ...
    if (!ids.length) {
        ...
        for (const id of READ_FIRST) {
            if (!(await exists(join(root, id)))) {
                add("warning", "context", `Missing ${id}. Session freshness is switched off for this project ` +
    ...
    const missing = (await readInputs(root, ids)).filter((i) => (i.fingerprint === ABSENT || i.fingerprint === UNREADABLE) && !covered.includes(i.id));
    if (!missing.length)
        return;
    add("error", "context", `${missing.length} record(s) every session must load ${missing.length === 1 ? "is" : "are"} ` +
 ...
 */
async function checkTrunk(root, declared, offline, add) {
    ...
    if (remotes === null) {
        add("warning", "context", `Could not ask git for this repo's remotes, so the session-freshness trunk ` +
    ...
    if (remotes.length && !remotes.includes(trunk.remote)) {
        add("error", "context", `The session-freshness trunk is "${trunk.remote}/${trunk.branch}" but this repo has no ` +
    ...
    if (!remotes.length) {
        add("error", "context", `This repo has no git remotes, so the session-freshness trunk ` +
    ...
    if (!declared) {
        ...
        if (others.length) {
            add("warning", "context", `No "context.trunk" in morpheus.json, and this repo has remotes besides origin ` +
    ...
    if (offline) {
        ...
        add("warning", "context", `Offline: did not check that "${trunk.remote}/${trunk.branch}" resolves. A declared trunk ` +
    ...
    if (observed.reason === "missing") {
        add("error", "context", `The session-freshness trunk "${trunk.remote}/${trunk.branch}" does not exist on the ` +
...
export async function doctor(opts) {
    const { root } = opts;
    const findings = [];
    const add = (severity, check, message) => findings.push({ severity, check, message });
    // --- manifest -----------------------------------------------------------
    let manifest = null;
    try {
        ...
        add("error", "manifest", `morpheus.json missing or invalid — ${err instanceof Error ? err.message.split("\n")[0] : err}`);
    ...
    const prefix = typeof manifest.prefix === "string" && /^[A-Z]{2,4}$/.test(manifest.prefix)
        ? manifest.prefix
        : undefined;
    if (manifest.prefix === undefined) {
        add("error", "prefix", 'No "prefix" — ids would collide with other projects. Add 2-4 uppercase letters.');
    }
    else if (!prefix) {
        add("error", "prefix", `"prefix" must be 2-4 uppercase letters; got ${JSON.stringify(manifest.prefix)}. ` +
    ...
    if (manifest.kind === undefined) {
        ...
        add("error", "kind", `"kind" must be one of ${Kind.options.join(", ")}; got ${JSON.stringify(manifest.kind)}. ` +
    ...
    if (contextBlock !== undefined && (typeof contextBlock !== "object" || contextBlock === null || Array.isArray(contextBlock))) {
        ...
        // fields.
        add("error", "context", `context is not an object, so every session-freshness setting in it is ignored and the ` +
    ...
    for (const [key, expected, ok] of [
        ["handle", "a string", (v) => typeof v === "string"],
        ["trunk", "a string", (v) => typeof v === "string"],
        ["requiredInputs", "an array", (v) => Array.isArray(v)],
    ]) {
        if (raw[key] !== undefined && !ok(raw[key])) {
            add("error", "context", `context.${key} is not ${expected}, so it is ignored entirely and the default applies. ` +
    ...
    if (handle && !(await exists(join(root, "hq", "team", `${handle}.md`)))) {
        ...
        add("error", "context", `morpheus.json declares context.handle "${handle}" but hq/team/${handle}.md does not ` +
    ...
    if (droppedInputs?.length) {
        add("error", "context", `context.requiredInputs has ${droppedInputs.length} entr${droppedInputs.length === 1 ? "y" : "ies"} ` +
    ...
    if (kind !== "internal") {
        for (const [path, marker] of MARKETING_BRIEFS) {
            ...
            if (content === null) {
                add("warning", "marketing", `Missing ${path} — run morpheus init to add the project-owned initialization brief.`);
            }
            else if (content.includes(`morpheus:template ${marker}`)) {
                add("warning", "marketing", `${path} is still the initialization scaffold — replace it with current project evidence before treating this area as ready.`);
            }
        }
    ...
    if (!entry) {
        ...
        add("error", "registry", `Registry says prefix ${entry.prefix} but morpheus.json says ${manifest.prefix}. The manifest wins.`);
    }
    // --- content ------------------------------------------------------------
    const productDir = join(root, "hq/product");
    if (await exists(productDir)) {
        for (const artifact of ["roadmap", "goals", "requests"]) {
            const { items, issues } = await parseArtifact(productDir, artifact);
            for (const i of issues)
                add("error", `pm:${artifact}`, i.message);
            if (prefix) {
    ...
    if (await exists(inboxDir)) {
        ...
        const files = (await readdir(inboxDir)).filter((f) => f.endsWith(".md") && !TEAM_RESERVED.has(f.toLowerCase()));
        if (files.length === 0) {
        ...
        for (const f of files) {
            ...
            const parsed = await parseInboxFile(join(inboxDir, f)).catch((err) => ({ issues: [{ message: `unreadable — ${err.message}` }] }));
            for (const i of parsed.issues)
                add("error", "inbox", `${f}: ${i.message}`);
        }
    }
    return findings;
}
export function formatFindings(findings, label) {
    ...
    const lines = findings.map((f) => `${f.severity === "error" ? "✗" : "!"} [${f.check}] ${f.message}`);
