...
export function buildReviewPrompt(ctx) {
    const parts = [ctx.persona.trim()];
    if (ctx.id && ctx.title) {
        parts.push(section(`What this change was supposed to do — ${ctx.id}`, `**${ctx.title}**\n\n${ctx.intent?.trim() || "_The item records no detail beyond its title._"}`));
    }
    else {
        parts.push(section("What this change was supposed to do", "This branch names no roadmap item, so there is no stated intent to compare against. " +
    ...
    if (ctx.acceptance) {
        parts.push(section("Acceptance criteria", `${ctx.acceptance.trim()}\n\n` +
    ...
    if (ctx.missingAcceptance) {
        parts.push(section("Acceptance criteria — missing", `${ctx.id ?? "The item"} declares \`acceptance: ${ctx.missingAcceptance}\`, but no such ` +
 ...
 */
export function acceptancePath(value) {
