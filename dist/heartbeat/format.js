...
export function formatBeat(beat) {
    const lines = [];
    lines.push(`Heartbeat — ${beat.inFlight.length}/${beat.ceiling} in flight, ${beat.blocked.length} blocked`, "");
    if (beat.pick) {
    ...
    if (beat.meetings.sinceLastNote !== null) {
        ...
        lines.push(...bullet("Meeting record:", [
    ...
    if (beat.ranked.length > 1) {
        lines.push(...bullet("Next up:", beat.ranked.slice(1, 4).map((c) => `${c.id.padEnd(8)} ${c.priority} ${c.title}`)));
    }
    return lines.join("\n").trimEnd();
}
