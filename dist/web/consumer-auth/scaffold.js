 ...
 */
export function unitTestPath(survey, base) {
    const suffix = survey.testRunner === "node" ? "test.mjs" : "node-test.mjs";
    return `__tests__/${base}.${suffix}`;
}
const UNIT_TEST_BASES = ["auth-config", "action-link", "request-safety", "user-profile"];
/**
...
export async function scaffoldConsumerAuth(opts) {
    ...
    if (tsconfig.kind === "note")
        notes.push(tsconfig.message);
...
export async function ensureEmulatorsBlock(root, rulesPath) {
    ...
    if (parsed.emulators !== undefined) {
        ...
        const conflicts = (current.auth?.port !== undefined && current.auth.port !== 9099) ||
 ...
 */
export async function ensureTsExtensionImports(root, webRoot) {
    ...
    try {
        ...
        return {
            kind: "note",
            message: "The app's tsconfig.json could not be parsed as plain JSON (comments, probably), so " +
