...
export async function scaffoldWeb(opts) {
    ...
    if (!survey.webAppExists) {
        ...
        notes.push("Created a Next.js app. Run `pnpm install` from the repository root before " +
    ...
    if (serverless && (opts.waitlist || opts.hq)) {
        notes.push("This app sets `output: \"export\"`, which builds HTML and nothing else — no route " +
    ...
    if (opts.waitlist && !serverless) {
        if (!opts.firebase) {
            notes.push("Skipped the waitlist: it writes to Firestore, and no Firebase project is known " +
            ...
            if (survey.testRunner) {
                ...
                if (survey.testRunner === "node") {
                    notes.push(`Add ${test.path} to the web app's \`test\` script — a \`node --test\` project ` +
                        "names its files explicitly, so a new one runs nowhere until it is listed.");
                }
            }
            else {
                notes.push("No test runner detected in the web app, so no test was generated. " +
            ...
            if (!survey.webAppExists) {
                ...
                notes.push("The home page was left alone. Render `<WaitlistForm source=\"hero\" />` where " +
    ...
    if (opts.hq && !serverless) {
        if (!opts.firebase) {
            notes.push("Skipped /hq: Google sign-in needs a Firebase project, and none is known yet. " +
            ...
            if (survey.hasRouteGate) {
                notes.push("An existing proxy.ts or middleware.ts was left untouched. Add the `/hq` " +
            ...
            notes.push("Grant the team their roles with `morpheus access sync`. Until that runs, a " +
    ...
    if (opts.firebase && !serverless) {
        ...
        if (survey.shared && !survey.shared.exportsWaitlist && survey.shared.hasExportsMap) {
            ...
            if (await addSharedExport(root, path))
                merged.push(`${path} (+./schema/waitlist)`);
        }
    }
    if (opts.waitlist && opts.firebase && !serverless) {
        ...
        if (rules.kind === "merged")
            merged.push(`${rules.path} (+waitlist deny block)`);
        if (rules.kind === "note")
 ...
 */
export async function mergeDependencies(root, webRoot, required) {
    ...
    const devCurrent = (manifest.devDependencies ?? {});
    const added = [];
    const next = { ...current };
    for (const [dependency, range] of Object.entries(required)) {
        if (current[dependency] || devCurrent[dependency])
            continue;
 ...
 */
async function addSharedExport(root, relativePath) {
 ...
 */
export async function addWaitlistRules(root, rulesPath) {
    if (!rulesPath) {
        return {
            kind: "note",
            message: "No deployed Firestore rules file is configured, so the waitlist deny block was " +
    ...
    if (/match\s+\/waitlist\//.test(existing))
        return { kind: "none" };
    const match = CATCH_ALL.exec(existing);
    if (!match) {
        return {
            kind: "note",
            message: `${rulesPath} does not carry the generated catch-all comment, so the waitlist ` +
