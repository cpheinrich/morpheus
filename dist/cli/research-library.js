...
async function readManifest(root) {
    ...
    try {
        return { path: manifestPath, value: JSON.parse(await readFile(manifestPath, "utf8")) };
    }
    catch (error) {
...
export async function initResearchLibrary(options) {
    try {
...
export async function runResearchLibrary(command, args, options) {
    try {
        ...
        return await run("python3", [script, ...(options.gcloud ? ["--gcloud", options.gcloud] : []), command, ...forwarded], {
            ...process.env,
...
async function ensureIgnored(root, localRoot) {
    const ignorePath = join(root, ".gitignore");
    let value = "";
    try {
        value = await readFile(ignorePath, "utf8");
    }
    catch { /* optional */ }
...
//# sourceMappingURL=research-library.js.map