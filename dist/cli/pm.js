...
import { renderGoals, renderRequests, writeIndex, writeStaticRoadmapReadme, } from "../pm/index-gen.js";
...
const INDEXED_KINDS = ["goals", "requests"];
...
async function validateKind(productDir, kind) {
    switch (kind) {
        case "roadmap": {
...
async function renderIndexKind(productDir, kind) {
    switch (kind) {
        case "goals": {
            const { items, issues } = await parseArtifact(productDir, "goals");
            return {
...
export async function index(productDir, check = false) {
    ...
    if (await exists(roadmapDir)) {
        ...
        if (changed) {
            stale++;
            console.log(`${check ? "✗ stale  " : "updated  "}${roadmapDir}/README.md — removed generated table`);
        }
        else {
...
export async function claim(productDir, id, cwd, sessionId = process.env.CODEX_THREAD_ID, offline = false) {
    if (!id) {
        console.error("Usage: morpheus pm claim RM-014");
        return 1;
    }
    try {
        if (offlineDeclared(offline))
            throw new Error("New claims require a verified remote; offline claiming is refused.");
...
export async function resume(id, cwd, sessionId = process.env.CODEX_THREAD_ID) {
    try {
...
export async function migrateIds(productDir, dryRun, repoRoot = process.cwd()) {
    ...
    if (!dryRun)
        console.log("Run `morpheus pm index` to refresh product README files.");
    return result.problems.length ? 1 : 0;
}
