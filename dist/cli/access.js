...
export async function sync(repoRoot, project, dryRun) {
    let manifest;
    try {
        manifest = ProjectManifest.parse(JSON.parse(await readFile(join(repoRoot, "morpheus.json"), "utf8")));
    }
    catch (err) {
    ...
    for (const r of results) {
        console.log(`${MARK[r.outcome] ?? " "} ${r.email.padEnd(30)} ${(r.role ?? "—").padEnd(10)} ${r.outcome}` +
            (r.detail ? `  (${r.detail})` : ""));
    }
