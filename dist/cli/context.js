import { CANONICAL_INPUTS, LEASE_TTL_MS, observeLease, } from "../session/lease.js";
 ...
 * from not asking.
 */
export async function refresh(root, offline = offlineDeclared()) {
    ...
    if (previous && !previous.remoteSha && lease.receipt.remoteSha) {
        ...
        if (behind === null) {
            ...
            for (const line of behind.slice(0, 20))
                console.log(`  ${line}`);
            if (behind.length > 20)
                console.log(`  … and ${behind.length - 20} more`);
        }
        else {
    ...
    if (previous?.remoteSha &&
        ...
        if (log === null) {
            ...
            console.log(`    git fetch ${trunk.remote} ${trunk.branch} && ` +
    ...
    if (previous) {
        ...
        if (moved.length || added.length) {
            ...
            for (const id of [...moved, ...added].sort())
                console.log(`  ${id}`);
            console.log("");
        }
    }
    ...
    if (lease.unresolvableInputs?.length) {
        ...
        for (const id of lease.unresolvableInputs)
            console.log(`    ${id}`);
    }
    if (trunkMissing) {
        ...
        console.log(`  \`${trunkMissing.remote}/${trunkMissing.branch}\` does not exist on the remote.`);
...
export async function status(root, offline = offlineDeclared()) {
    const now = new Date();
    ...
    if (issue)
        console.log(`  ! ${issue}`);
    if (!written)
        console.log(`  ! This verdict was not persisted — the lease on disk is older than it.`);
    if (trunkMissing) {
        console.log(`  ! \`${trunkMissing.remote}/${trunkMissing.branch}\` does not exist — set \`context.trunk\` in morpheus.json.`);
    }
 ...
 */
async function sinceReceipt(root, receipt, now) {
    const wt = await worktreeRoot(root);
    const policy = await projectPolicy(wt);
 ...
 */
export async function install(root, opts) {
    const repairs = await installContext(root, { write: !opts.check, handle: opts.handle });
    const mark = (r) => r.outcome === "blocked" ? NO : r.outcome === "present" ? "·" : OK;
 ...
 */
export async function guard(root, action, reach, offline) {
