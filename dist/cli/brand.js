...
import { ANSWERS_FILE, readAnswers, readAnswersDetailed, writeAnswers, } from "../brand/answers.js";
import { checkDrift, generateBrand } from "../brand/generate.js";
import { BrandAnswers, QUESTIONS } from "../brand/questions.js";
...
async function ask(rl, q, prior) {
    console.log(`\n\x1b[1m${q.prompt}\x1b[0m`);
    console.log(`\x1b[2m${q.why}\x1b[0m`);
    if (q.example)
        console.log(`\x1b[2me.g. ${q.example}\x1b[0m`);
    ...
    if (q.list) {
        ...
        if (items.length)
            return items;
        return Array.isArray(prior) ? prior : undefined;
    }
...
export async function check(opts) {
    ...
    const { derived, seeded, missing } = await checkDrift(opts.brandDir, opts.name, opts.prefix, answers);
    ...
    if (seeded.length) {
        ...
        for (const f of seeded)
            console.error(`  ${f}`);
        console.error("\x1b[2m  These were generated once and are now yours. Morpheus will not\n" +
...
export async function init(opts) {
    ...
    console.log("\x1b[2mEight questions. Answer in a sentence; nothing here writes a TODO, so a\n" +
    ...
    const { files, skipped, stale } = await generateBrand(opts.brandDir, opts.name, opts.prefix, parsed.data, { refresh: opts.refresh });
    if (files.length) {
...
export async function build(opts) {
    const { answers, issues, exists } = await readAnswersDetailed(opts.brandDir);
    if (!exists) {
        console.error(`No ${ANSWERS_FILE} in ${opts.brandDir}.\n` +
    ...
    const { files, skipped, stale } = await generateBrand(opts.brandDir, opts.name, opts.prefix, answers, { refresh: true });
    if (files.length) {
