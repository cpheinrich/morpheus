...
import { buildBrief } from "../voice/brief.js";
...
export async function knowledge(root, out) {
    const m = await readManifest(root);
    if (!m.prefix) {
        console.error('No "prefix" in morpheus.json. The explainer describes this project\'s id scheme,\n' +
            "so it cannot be written without one.");
        return 1;
    }
    const path = join(root, out ?? KNOWLEDGE_PATH);
    await write(path, buildKnowledge({
 ...
 */
async function openInboxItems(root) {
    ...
    for (const file of files) {
        try {
            ...
            for (const item of parsed.items) {
                if (item.state === "open")
                    items.push({ n: item.n, title: item.title });
            }
        }
        catch {
...
export async function brief(opts) {
    ...
    const contents = opts.full && m.prefix
        ? `${buildKnowledge({
            name,
            prefix: m.prefix,
            ...(m.kind ? { kind: m.kind } : {}),
