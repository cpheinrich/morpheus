import { BrandAnswers, QUESTIONS, type Question } from "./questions.js";
...
const ANCHOR = (key: string): string => `<!-- morpheus:q ${key} -->`;
const ANCHOR_RE = /^<!--\s*morpheus:q\s+(\S+)\s*-->$/;

const isList = (q: Question): boolean => q.list === true;

function section(q: Question, value: unknown): string {
  ...
  lines.push(`> ${q.why}`);
  if (q.example) lines.push(`>`, `> e.g. ${q.example}`);
  lines.push("");
...
export function renderAnswersMd(
  name: string,
 ...
 */
function answerLines(block: string[]): string[] {
  return block
    .map((l) => l.trimEnd())
...
export function parseAnswersMd(text: string): ParseResult {
