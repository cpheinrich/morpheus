...
type Ownership = "derived" | "authored";
...
const normalise = (text: string): string => (text.endsWith("\n") ? text : `${text}\n`);

function moodboardReadme(): string {
  return [
    "# Moodboard input",
    "",
...
function researchReadme(): string {
  return [
    "# Brand concept review",
    "",
    "`brand.html` is the durable comparison surface for the exploration, not a one-off screenshot. It keeps each concept's design system, app-home mock, public marketing mock, typography study, graphic-language study, and substantial Compare All view in one place while the underlying copy and test content stay fixed.",
    "",
    ...
    "```",
    "",
    `Mark each stable package with \`${CONCEPT_REVIEW_CONCEPT_ATTRIBUTE}="stable-name"\`, and mark the six rendered panels with \`${CONCEPT_REVIEW_VIEW_ATTRIBUTE}="system"\`, \`home\`, \`marketing\`, \`type\`, \`graphics\`, and \`compare\`. Those portable markers let Morpheus validate the durable review without constraining its layout or JavaScript.`,
    "",
...
function explorePrompt(name: string, prefix: string, vibes: string): string {
  ...
  return [
    ...
    "",
    "The page needs concept tabs plus these six views, and the same product content must appear in each direction so the review is apples-to-apples:",
    "",
    ...
    "5. **Graphics** — a focused study of the concept's illustration, diagram, icon, or image-making language. Show multiple candidates at a consistent scale, including at least one restrained/simple composition and one denser composition, plus enough placement context to judge crop, background, and palette behavior.",
    "6. **Compare All** — a substantial side-by-side review board: art or hero treatment, palette, type, motif, at least one mobile/product snapshot, UI primitives, and the concept's distinction. A single color rectangle is not enough to make a real decision.",
    "",
...
function finalizePrompt(name: string, selection: string): string {
...
function plan(name: string, prefix: string, vibes: string): PlannedFile[] {
  return [
...
export async function writeFinalizePrompt(opts: {
  ...
  if (error) return { error: `Cannot finalize before ${CONCEPT_REVIEW_FILE} is ready — ${error}.` };
