...
function addPathMatches(value: string, found: Set<string>): void {
  for (const match of value.matchAll(PATH_LIKE)) found.add(match[1]!);
}

/** Paths carried by the two URL shapes the reviewer emits for findings. */
function pathsFromUrl(raw: string, found: Set<string>): void {
  try {
    ...
    if (url.hostname === "github.com") {
      const repoPath = decodeUrlText(url.pathname).match(
        /^\/[^/]+\/[^/]+\/(?:blob|tree)\/[^/]+\/(.+)$/,
      )?.[1];
      if (repoPath) addPathMatches(repoPath, found);
    }
...
export function pathsMentioned(reviewBody: string): string[] {
  ...
  for (const match of reviewBody.matchAll(URL_LIKE)) pathsFromUrl(match[0], found);
