 ...
 */
export async function projectCommands(root: string): Promise<string[]> {
  const commands: string[] = [];
  const pkg = await readJson<{ scripts?: Record<string, string>; packageManager?: string }>(
...
export function reviewPacket(opts: {
  ...
  if (ticket.id) {
    ...
    if (ticket.intent) lines.push(ticket.intent);
  } else {
    lines.push(
      "Ticket: unclaimed change — review against the PR title and body; no roadmap item is required for this review.",
    );
  }
  if (ticket.acceptance) lines.push(`Acceptance: ${ticket.acceptance}`);
    ...
    lines.push(
      "Acceptance: none declared — review against the ticket context and the PR description. That is a supported state, not a gap to fill by creating an item.",
    );
  }
  return lines.join("\n");
}
