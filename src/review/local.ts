...
function onlyWorklogChanges(root: string, older: string, newer: string, worklog: string): void {
...
function regularDocumentation(root: string, older: string, newer: string): void {
  const paths = changedPaths(root, older, newer);
  if (!paths.length || paths.some(p => !documentationPath(p))) throw new Error("documentation integration contains non-documentation paths");
  for (const ref of [older, newer]) {
    for (const path of paths) {
      const entry = git(root, ["ls-tree", ref, "--", path]);
      if (entry && !entry.startsWith("100644 blob ")) throw new Error("documentation integration requires regular non-executable files");
    }
  }
}

/** Verify Git's merge result, not an author's assertion that the integration was harmless. */
function checkDocumentationIntegrations(root: string, record: LocalReviewRecord, worklog: string, head: string, clearedBase: string): string {
  ...
  for (const integration of record.documentationIntegrations ?? []) {
    ...
    if (parents.length !== 2 || parents[1] !== integration.base) throw new Error("documentation integration must be an explicit two-parent trunk merge");
    ...
    for (const commit of incoming) {
      ...
      if (sourceParents.length !== 1) throw new Error("documentation integration requires linear reviewed trunk commits");
      const parent = sourceParents[0]!;
      regularDocumentation(root, parent, commit);
      const sourcePath = sources.get(commit)!;
      if (!changedPaths(root, parent, commit).includes(sourcePath)) throw new Error("documentation source must carry its own review record");
...
function checkFollowUpChain(root: string, record: LocalReviewRecord): void {
  ...
  for (const turn of followUpTurns(record)) {
    if (!isAncestor(previousCommit, turn.commit)) throw new Error("follow-up turns must be recorded in commit order, each covering a descendant of the last");
    if (turn.base) {
      if (!turn.scopeReason) throw new Error("base integration requires an explicit follow-up scope reason");
      if (!isAncestor(previousBase, turn.base) || !isAncestor(turn.base, turn.commit)) throw new Error("a follow-up base must move forward along trunk and precede that turn's commit");
...
export function checkLocalReview(opts: { root: string; body: string; labels: string[]; head: string; base: string }): Finding[] {
  try {
    ...
    if (git(opts.root, ["merge-base", opts.base, opts.head]) !== integratedBase) throw new Error("review base is stale; reconcile the base and review coverage explicitly");
