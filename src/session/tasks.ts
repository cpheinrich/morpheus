...
interface TaskBinding { root: string; task: string; pending?: boolean }

export async function bindTask(cwd: string, task: string, sessionId?: string, pending = false): Promise<void> {
  if (!sessionId) return;
...
export async function boundTask(cwd: string, sessionId?: string): Promise<TaskBinding | null> {
  if (!sessionId) return null;
  const { common } = await checkoutIdentity(cwd);
  let raw: string;
  try { raw = await readFile(join(common, "morpheus-sessions", `${sessionKey(sessionId)}.json`), "utf8"); }
  catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return null; throw error; }
  const binding = JSON.parse(raw) as TaskBinding;
  if (typeof binding.root !== "string" || typeof binding.task !== "string" || !ROADMAP_ID.test(binding.task)) throw new Error("Invalid session task association.");
...
async function destination(root: string, task: string): Promise<string> {
...
export async function prepareTask(root: string, productDir: string, id: string): Promise<string | null> {
  if (!ROADMAP_ID.test(id)) throw new Error(`Invalid roadmap ID: ${id}`);
  const checkout = await checkoutIdentity(root);
  const branch = await git(root, ["rev-parse", "--abbrev-ref", "HEAD"]);
  if ((await findClaims(id, root)).length) throw new Error(`${id} is already claimed. Use morpheus pm resume ${id} to continue it explicitly.`);
  const { items } = await parseArtifact(productDir, "roadmap");
  const item = items.find(i => i.data.id === id);
  if (!item) throw new Error(`No roadmap item ${id} in ${productDir}.`);
  if (item.data.status === "shipped" || item.data.status === "dropped") throw new Error(`${id} is ${item.data.status}; nothing to claim.`);
  const itemRel = relative(checkout.root, await realpath(item.path));
  if (itemRel.startsWith("..")) throw new Error("The roadmap must belong to this repository.");
  ...
  if (checkout.linked && branch === "HEAD" && head === sha && !trackedChanges) return null;
  ...
  if (!tracked) {
    ...
    if (currentItems.items.some(i => i.data.id === id)) throw new Error(`${id} now exists on trunk. Read ${target} before claiming it.`);
...
export async function resumeTask(root: string, id: string, sessionId?: string): Promise<string> {
  if (!ROADMAP_ID.test(id)) throw new Error(`Invalid roadmap ID: ${id}`);
  const branches = await findClaims(id, root);
  if (branches.length !== 1) throw new Error(`Expected one remote claim for ${id}; found ${branches.length}. Resolve the task identity before resuming.`);
  ...
  for (const entry of listing.split("\0\0")) {
    ...
    if (fields.includes(`branch refs/heads/${branch}`)) {
      ...
      if (!target) throw new Error("Git returned a worktree without its path.");
  ...
  const target = await destination(root, id);
  if (!local) await git(root, ["fetch", "--no-write-fetch-head", "--", "origin", `refs/heads/${branch}:refs/heads/${branch}`]);
