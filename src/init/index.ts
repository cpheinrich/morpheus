...
export async function scaffold(root: string, seed: Seed): Promise<InitResult> {
  ...
  for (const repair of await installContext(root, { write: true, handle: seed.owner })) {
    if (repair.target === "morpheus.json") continue; // written above, with the handle
    if (repair.outcome === "created") written.push(repair.target);
  ...
  const xcodeProject = (await readdir(iosDirectory).catch(() => [])).find((entry) =>
  ...
  if (existing.includes("# Morpheus")) {
    ...
    const missingBrandIgnore = t.BRAND_EXPLORATION_IGNORE_RULES.filter(
