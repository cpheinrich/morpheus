...
export type VisualEvidencePolicy =
  ...
  | { state: "configured"; config: VisualEvidenceConfig }
 ...
 */
export const DEFAULT_VISUAL_EVIDENCE: VisualEvidenceConfig = {
  enabled: true,
  include: [
...
const Pattern = z
  ...
  .refine(
    ...
    "must not leave the repository with '..'",
  );
...
export function visualEvidencePolicy(manifest: unknown): VisualEvidencePolicy {
  const root = object(manifest);
  if (!root) return { state: "invalid", message: "morpheus.json is not an object" };

  const review = root["review"];
  if (review === undefined) return { state: "absent" };
  const reviewObject = object(review);
  if (!reviewObject) return { state: "invalid", message: "review must be an object" };
...
export function unreadableVisualEvidencePolicy(message: string): VisualEvidencePolicy {
  return { state: "invalid", message: `could not read morpheus.json: ${message}` };
}

const FRONTEND_LIKE =
  /(?:\.(?:avif|css|gif|ico|jpeg|jpg|jsx|png|scss|storyboard|svg|swift|tsx|webp)$|\.xcassets\/)/i;
const GITHUB_ATTACHMENT =
  /https:\/\/(?:github\.com\/user-attachments\/assets\/[A-Za-z0-9-]+|user-images\.githubusercontent\.com\/[^\s)]+)/gi;
const GITHUB_ATTACHMENT_ON_LINE =
  /https:\/\/(?:github\.com\/user-attachments\/assets\/[A-Za-z0-9-]+|user-images\.githubusercontent\.com\/[^\s)]+)/i;
const HTTPS_URL = /https:\/\/[^\s)<>"']+/gi;

function matchesAllowedPrefix(value: string, prefixes: string[]): boolean {
  try {
...
function evidenceUrls(content: string, prefixes: string[]): string[] {
...
function paths(paths: string[]): string {
...
function sectionContent(body: string, heading: string): string | null {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (match[2]?.toLowerCase() === want) start = i + 1;
  }
...
export function checkVisualEvidence(input: {
  ...
  if (policy.state === "absent") {
    return likely.length
      ? [{
          ...
          rule: "visual-evidence-config",
          message:
            `Front-end-looking files changed (${paths(likely)}), but this legacy manifest has no ` +
  ...
  const required = changedFiles.filter(
    (path) => matches(path, include) && !excluded.includes(path),
  );
  const unclassified = likely.filter(
    (path) => !required.includes(path) && !excluded.includes(path),
  );
  ...
  if (unclassified.length) {
    findings.push({
      ...
      message:
        `Front-end-looking files sit outside the declared path contract: ${paths(unclassified)}. ` +
        "Evidence is not required for them; update include or exclude if the contract drifted.",
    });
  }

  if (!required.length) return findings;
  ...
  if (!content) {
    findings.push({
      ...
      rule: "visual-evidence",
      message:
        `Declared front-end paths changed (${paths(required)}). Add a non-empty ` +
  ...
  if (!attachments.length) {
    findings.push({
      ...
      rule: "visual-evidence",
      message:
        'The "## Visual evidence" section has no approved evidence URL. Paste a GitHub attachment ' +
  ...
  const recording = content
    ...
    .some(
      (line) =>
        /(?:screen\s+recording|recording|video)(?:\*\*)?\s*:/i.test(line) &&
  ...
  if (!recording) {
    findings.push({
      ...
      message:
        "Visual evidence is attached. A screen recording is preferred; label its line `Recording:`. " +
