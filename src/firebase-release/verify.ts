...
export function indexKey(value: unknown, expected = false): string {
  ...
  if (expected) {
    assert(Object.keys(index).every((key) => ["collectionGroup", "queryScope", "fields"].includes(key)),
      "Unsupported index option requires a reviewed readiness implementation");
  }
