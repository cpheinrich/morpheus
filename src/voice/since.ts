 ...
 */
export function latestHandoffDate(filenames: string[]): string | null {
 ...
 */
export async function readSince(root: string): Promise<Since> {
  ...
  try {
    ...
    if (date) args.push(`--since=${date} 00:00:00`);
    else args.push("-20");
