...
export type ResearchLibraryBundle = {
  bucket: string; object: string; sha256: string; bytes: number; files: number;
};
export type ResearchLibraryReader = {
  bucket: string; object: string; sha256: string; bytes: number;
  sourceBundleSha256: string; format: typeof RESEARCH_LIBRARY_READER_FORMAT;
};
export type ResearchLibraryBook = {
  schemaVersion: typeof RESEARCH_LIBRARY_BOOK_SCHEMA;
  slug: string; title: string; authors: string[]; sourceDirectory: string;
  edition?: string; publisher?: string; year?: number; isbn?: string[]; language?: string;
  bundle: ResearchLibraryBundle; reader: ResearchLibraryReader;
};
export type ResearchLibraryCatalogIssue = { path: string; message: string };
export type ResearchLibraryCatalog = { books: ResearchLibraryBook[]; issues: ResearchLibraryCatalogIssue[] };
export type ResearchLibraryContract = { bucket: string; objectPrefix?: string; catalogDir?: string };

export function parseResearchLibraryBook(
  value: unknown,
  contract: ResearchLibraryContract,
): ResearchLibraryBook | null {
  if (!isRecord(value) || value.schemaVersion !== RESEARCH_LIBRARY_BOOK_SCHEMA) return null;
  ...
  if (!isSlug(slug) || typeof title !== "string" || !title.trim()) return null;
  if (!isStringArray(authors) || authors.length === 0) return null;
  if (typeof sourceDirectory !== "string" || !sourceDirectory || sourceDirectory.includes("/") ||
      sourceDirectory === "." || sourceDirectory === "..") return null;
  ...
  if (bundle.bucket !== contract.bucket || typeof bundle.sha256 !== "string" ||
      ...
      !isPositiveInteger(bundle.bytes) || !isPositiveInteger(bundle.files)) return null;
  if (!isRecord(reader)) return null;
  if (reader.bucket !== contract.bucket || typeof reader.sha256 !== "string" ||
      ...
      reader.format !== RESEARCH_LIBRARY_READER_FORMAT) return null;
  const optionalStrings = [value.edition, value.publisher, value.language];
  if (optionalStrings.some((field) => field !== undefined &&
      (typeof field !== "string" || !field.trim()))) return null;
  if (value.year !== undefined && !isPositiveInteger(value.year)) return null;
  if (value.isbn !== undefined && !isStringArray(value.isbn)) return null;
