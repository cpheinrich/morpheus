...
export type MorpheusCommandRunner = (
  command: string,
  args: string[],
  cwd: string,
) => Promise<MorpheusCommandResult>;
...
export type MorpheusInstallRelation =
  | "current"
  | "ahead"
  | "stale"
  | "dirty"
...
function parseReceipt(raw: string): InstallReceipt | null {
  try {
    const value = JSON.parse(raw) as Partial<InstallReceipt>;
    if (
      value.schema !== 1 ||
...
async function readMorpheusInstallStatus(
  opts: MorpheusStatusOptions,
): Promise<MorpheusInstallStatus> {
  const runner = opts.runner ?? runMorpheusCommand;
  const source = resolve(opts.packageRoot ?? MORPHEUS_PACKAGE_ROOT);
  const receipt = parseReceipt(
    await readFile(join(source, INSTALL_RECEIPT), "utf8").catch(() => ""),
  );
  const checkout = receipt ? null : await exactCheckout(source, runner);
  const kind: MorpheusInstallKind = receipt ? "package" : checkout ? "checkout" : "unknown";
  ...
  const remote = await runner(
    "git",
    ["ls-remote", "--exit-code", MORPHEUS_REMOTE, "refs/heads/main"],
    source,
  );
  ...
  if (checkout) {
    const contains = await runner(
      "git",
      ["merge-base", "--is-ancestor", remoteSha, installedSha],
      source,
    );
    if (contains.code === 0) {
      return {
...
export function morpheusInstallStatus(
  opts: MorpheusStatusOptions = {},
): Promise<MorpheusInstallStatus> {
  const cache = !opts.runner && !opts.packageRoot && opts.offline !== true;
  if (!cache) return readMorpheusInstallStatus(opts);
...
export function formatMorpheusInstallStatus(status: MorpheusInstallStatus): string {
  ...
  if (status.relation === "dirty") {
    return (
      `${no} Linked Morpheus source has local changes (${status.source})\n` +
      "  `morpheus self update` installs current main separately and does not touch them."
    );
  }
  if (status.relation === "stale") {
    return (
      `${no} Morpheus ${short(status.installedSha)} differs from current main ${short(status.remoteSha)}\n` +
  ...
  return (
    `${maybe} Morpheus installation provenance could not be verified (${status.source})\n` +
...
function failure(command: string, result: MorpheusCommandResult): MorpheusInstallError {
  return new MorpheusInstallError(
    `${command} failed: ${(result.stderr || result.stdout || `exit ${result.code}`).trim()}`,
  );
}
...
export async function installCurrentMorpheus(
  sourceRoot: string,
  opts: MorpheusInstallOptions = {},
): Promise<MorpheusInstallResult> {
  const runner = opts.runner ?? runMorpheusCommand;
  ...
  if (checkout.head !== remoteSha) {
    throw new MorpheusInstallError(
      `The source checkout is ${short(checkout.head)}, not current main ${short(remoteSha)}.`,
    );
  }
  ...
  try {
    const packed = await requireSuccess(
      runner,
      "npm",
      ["pack", "--pack-destination", packDir, "--json", "--cache", join(packDir, "npm-cache")],
      source,
    );
    let filename = "";
    try {
    ...
    if (!filename) throw new MorpheusInstallError("npm pack did not report a package filename.");
    await requireSuccess(
      runner,
      "npm",
      ["install", "--global", join(packDir, filename), "--cache", join(packDir, "npm-cache")],
      source,
    );
    ...
    if (!stat?.isDirectory() || stat.isSymbolicLink()) {
      throw new MorpheusInstallError(
        `Global ${MORPHEUS_PACKAGE} is not a standalone directory at ${installedRoot}.`,
      );
    }
...
export async function updateMorpheus(
  opts: MorpheusInstallOptions = {},
): Promise<MorpheusInstallResult> {
  const runner = opts.runner ?? runMorpheusCommand;
  ...
  try {
    await requireSuccess(
      runner,
      "git",
      ["clone", "--depth", "1", "--branch", "main", "--single-branch", MORPHEUS_REMOTE, clone],
      parent,
    );
