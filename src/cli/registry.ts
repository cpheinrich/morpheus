...
async function manifest(root: string): Promise<Manifest | null> {
  try {
...
export async function list(): Promise<number> {
  ...
  for (const p of reg.projects) {
    console.log(
      `${p.prefix.padEnd(5)} ${p.name.padEnd(14)} ${(p.kind ?? "").padEnd(9)} ${p.path}`,
    );
  }
  return 0;
}
...
export async function add(cwd: string, prefixArg?: string): Promise<number> {
...
export async function remove(name: string): Promise<number> {
  try {
