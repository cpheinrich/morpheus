...
export async function refresh(root: string, offline = offlineDeclared()): Promise<number> {
  ...
  if (sourceAlignment) {
    if (sourceAlignment.status === "advanced") {
      ...
      console.log(
        `  Re-read the changed source and canonical records, then run ` +
    ...
    if (sourceAlignment.reason === "stale_branch") {
      console.error(
        `${NO} ${branch} does not contain current ${trunkRef} ` +
          `(${sourceAlignment.trunkSha.slice(0, 7)}).`,
      );
      console.error(
        `  Merge or rebase the trunk into this branch, re-read the changed source and records, ` +
          `then refresh again.`,
      );
    } else if (sourceAlignment.reason === "dirty_trunk") {
      console.error(`${NO} ${branch} is behind ${trunkRef}, but the working tree is dirty.`);
      console.error(
        `  Preserve or commit the local changes before updating the trunk, then refresh again.`,
      );
      ...
      console.error(
        `  Reconcile the branch deliberately, re-read the changed source and records, ` +
          `then refresh again.`,
      );
    } else if (sourceAlignment.reason === "fetch_failed") {
      console.error(
        `${NO} ${trunkRef} was observed but could not be fetched, ` +
